...
suite('Functional Tests', function() {
    suite("10 Functional tests", function() {
        ...
        test("Viewing te 10 most recent threads with 3 replies each: GET request to /api/threads/{board}", function (done) {
            chai
                .request(server)
        ...
        test("Deleting a thread with the incorrect password: DELETE request to /api/treads/{board} with invalid password", function (done) {
            chai
                .request(server)
                .delete("/api/treads/test-board")
                .set("content-type", "application/json")
        ...
        test("Reporting a thread: PUT request to /api/treads/{board}", function (done) {
            chai
                .request(server)
                .put("/api/treads/test-board")
                .set("content-type", "application/json")
        ...
        test("Creating a new reply: POST  request to /api/replies/{board}", function (done) {
            chai
                ...
                .end(function (err, res) {
                    assert.equal(res.status, 200);
                    assert.equal(res.body.treads[0].replies[0].text, "test reply");
        ...
        test("Viewing a single thread with all replies: GET request to /api/replies/{board}", function (done) {
            chai
                ...
                .end(function (err, res) {
                    assert.equal(res.status, 200);
                    console.log("test get whole thread body");
                    assert.equal(res.body._id, "test text");
                    assert.equal(res.body.text, "test text");
